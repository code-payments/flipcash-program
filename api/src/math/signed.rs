...
/// A `SignedPreciseNumber` represents a signed 192-bit fixed-point number with 18 decimal places of precision.
///
/// This struct extends [`PreciseNumber`] by adding a `bool` flag to indicate whether the value is negative,
/// enabling full support for signed decimal arithmetic.
///
/// ### Internal Representation
/// - The magnitude is stored as a [`PreciseNumber`], which wraps a [`InnerUint`] representing a 192-bit unsigned integer scaled by 10¹⁸.
/// - The `is_negative` flag determines the sign of the number.
///
/// ### Interpretation
/// The real-world value of a `SignedPreciseNumber` is:
/// ```text
/// value = (is_negative ? -1 : 1) × (magnitude / 10^18)
/// ```
///
/// ### Examples:
/// - `value = PreciseNumber::from_u192([1_000_000_000_000_000_000, 0, 0]), is_negative = false` → 1.0
/// - `value = PreciseNumber::from_u192([5_000_000_000_000_000_000, 0, 0]), is_negative = true`  → -5.0
///
